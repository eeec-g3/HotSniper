	...
	: SchedulerPinnedBase(thread_manager, SubsecondTime::NS(Sim()->getCfg()->getInt("scheduler/pinned/quantum"))), m_interleaving(Sim()->getCfg()->getInt("scheduler/pinned/interleaving")), m_next_core(0)
{
	...
	if (queuePolicy == "priority")
	{
		...
		if (randomPriority == true)
		{
			for (int taskIterator = 0; taskIterator < numberOfTasks; taskIterator++)
			{
				...
				cout << "[Scheduler]: Setting Priority for Task " << taskIterator << " (" + openTasks[taskIterator].taskName + ")"
	...
	if (distribution == "uniform")
	{
		...
		for (int taskIterator = 0; taskIterator < numberOfTasks; taskIterator++)
		{
			...
			cout << "[Scheduler]: Setting Arrival Time for Task " << taskIterator << " (" + openTasks[taskIterator].taskName + ")"
				 << " to " << time << +" ns" << endl;
...
void SchedulerOpen::threadExit(thread_id_t thread_id, SubsecondTime time)
{
	...
	if (thread_id < numberOfTasks)
	{
		...
		cout << "\n[Scheduler][Result]: Task " << app_id << " (Response/Service/Wait) Time (ns) "
			 << " :\t" << time.getNS() - openTasks[app_id].taskArrivalTime << "\t" << time.getNS() - openTasks[app_id].taskStartTime << "\t" << openTasks[app_id].taskStartTime - openTasks[app_id].taskArrivalTime << "\n";
	}
